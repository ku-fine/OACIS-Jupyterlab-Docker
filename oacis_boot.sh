...
usage() {
    ...
    echo "  -j PORT (default: 8888) : port used for JupyterLab"
...
# build the docker image, matching the OACIS version
if [ -n "${OACIS_VERSION}" ]; then
  ...
  docker build . -t ku-fine/oacisjupyterlab:${OACIS_IMAGE_TAG} --build-arg OACIS_VERSION=${OACIS_VERSION}
  cd $SCRIPT_DIR
fi
